...
for i in range(len(sentences)):
    waste = re.sub('[^a-zA-Z]', ' ', sentences[i])
    waste = waste.lower()
    waste = waste.split()
    words = [lemmatizer.lemmatize(word) for word in waste if word not in set(stopwords.words('english'))] #set helps in taking unique stop words in english
    waste = ' '.join(waste)
    corpus.append(waste)
