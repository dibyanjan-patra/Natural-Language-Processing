...
import nltk
from nltk.stem import PorterStemmer  #required for stemming purpose
...
sentences = nltk.sent_tokenize((paragraph))  ## breaking into sentences
stemmer = PorterStemmer()

#stopwords.words('english') #shows stop words in english language
#stopwords.words('german')  #shows stop words in german language

#Stemming/ to remove stop words from sentences
for i in range(len(sentences)):   #range will be 0 to 32
    words = nltk.word_tokenize(sentences[i])   #converting to words for every input sentence
    words = [stemmer.stem(word) for word in words if word not in set(stopwords.words('english'))] #set helps in taking unique stop words in english
    sentences[i] = ' '.join(words)

# Because of stemming , some words are nt completely meaningful in sentences part
